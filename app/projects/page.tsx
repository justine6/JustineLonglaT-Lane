...
function sortTopics(topics: string[]): string[] {
  ...
  return [...topics].sort((a, b) => {
    const ai = withIndex.has(a) ? withIndex.get(a)! : TOPIC_ORDER.length + a.charCodeAt(0);
    const bi = withIndex.has(b) ? withIndex.get(b)! : TOPIC_ORDER.length + b.charCodeAt(0);
    return ai - bi;
  });
}
...
export default function ProjectsPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-4 py-10 text-slate-50 sm:px-8 lg:px-16">
      <section className="mx-auto flex max-w-6xl flex-col gap-8">
        ...
        {FEATURED_MAIN && (
          <section className="rounded-2xl border border-slate-800 bg-gradient-to-r from-slate-900 via-slate-900 to-slate-800 p-6 shadow-xl shadow-slate-900/70">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <div className="rounded-xl bg-slate-900/80 px-4 py-3 text-xs text-slate-200">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {sortedTopics.map((topic) => (
              <button
                key={topic}
                type="button"
                className="rounded-full border border-slate-700 bg-slate-900 px-3 py-1 text-xs text-slate-200 transition hover:border-emerald-400 hover:text-emerald-200"
              >
                {topic}
              </button>
            ))}
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-slate-400">
            <div className="flex items-center gap-2">
              ...
              <div className="inline-flex rounded-full bg-slate-900 p-1">
                <span className="rounded-full bg-slate-700 px-3 py-1 text-xs font-semibold text-slate-50">
                  Grid
                </span>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="inline-flex items-center gap-1 rounded-full bg-slate-900 px-3 py-1">
                <span className="text-xs text-slate-100">Featured first</span>
              </div>
            </div>
          </div>
        </section>
