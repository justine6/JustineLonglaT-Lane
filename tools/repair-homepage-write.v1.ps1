# tools/repair-homepage-write.v1.ps1
...
$page   = Join-Path $appDir "page.tsx"

if(!(Test-Path -LiteralPath $appDir)){ throw "Folder not found: app/" }
if(Test-Path -LiteralPath $page){ Backup $page }

# Entire homepage with HeroBanner injected under <main>
$homepage = @'
...
export default function HomePage() {
  return (
    <main className="min-h-screen bg-white px-4 py-8 pt-20 text-gray-900 dark:bg-gray-950 dark:text-gray-100 sm:px-6 sm:py-10 md:pt-24">
      ...
      {/* ✅ Homepage-only banner under the top bar */}
      <HeroBanner />
...
# Write with UTF8 (CRLF friendly)
Set-Content -LiteralPath $page -Value $homepage -Encoding UTF8
Write-Host "Wrote app/page.tsx"

# Clear cache and warn if required images missing
...
$banner = Join-Path $root "public/brand/justine-banner.png"
$logo   = Join-Path $root "public/brand/justine-logo.png"
if(-not (Test-Path -LiteralPath $banner)){ Write-Warning "Missing required image: public/brand/justine-banner.png" }
if(-not (Test-Path -LiteralPath $logo)){   Write-Warning "Missing required image: public/brand/justine-logo.png" }

Write-Host "`nRepair complete. Start dev: npm run dev"
