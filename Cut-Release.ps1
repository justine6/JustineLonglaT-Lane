<# 
  ...
  Notes
  - Only SemVer tags (vX.Y.Z or X.Y.Z) are considered for bumping.
  - Set $env:FORCE_LAST_SEMVER to pin the "since" tag if needed.
  - Requires: git, gh, PowerShell 7+.

  Examples
    .\Cut-Release.ps1 -Stage prepare -Bump auto -Yes `
      -ChangelogScript .\scripts\Generate-Changelog.ps1
    .\Cut-Release.ps1 -Stage release -Bump minor -Yes
#>
...
param(
  ...
  # If omitted: inferred (main+clean -> release, otherwise prepare)
  [ValidateSet('prepare','release')]
  [string]$Stage,

  [switch]$DryRun,                 # print actions only
  [switch]$Yes,                    # non-interactive "yes"
...
# --- Small runner that respects -DryRun ---
function Run([string]$cmd) {
  if ($script:DryRun) { Write-Host "DRYRUN> $cmd" -ForegroundColor Yellow }
...
Write-Host ("🔧 Stage: {0}  |  Bump: {1}  |  Next: {2}  |  From: {3}  |  Branch: {4}" -f $Stage,$Bump,$next,$lastTag,$branch) -ForegroundColor Cyan
...
if ($Stage -eq 'prepare') {
  ...
  if (Test-Path $ChangelogScript) {
    if ($DryRun) { Write-Host "DRYRUN> & $ChangelogScript" -ForegroundColor Yellow }
    else { & $ChangelogScript }
  } else {
